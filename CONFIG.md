...
1. **Файл конфигурации** (`config.json`) — основной способ
2. **Переменные окружения** (переопределяют значения по умолчанию до загрузки config.json)
3. **Параметры командной строки** (переопределяют всё)
