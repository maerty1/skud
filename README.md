...
1. **Файл config.json** (рядом с исполняемым файлом) — основной способ
2. **Переменные окружения** (переопределяют дефолты)
...
```
Создастся `config.json` рядом с exe. Отредактируйте его под ваше окружение. Либо скопируйте `bin/config.json.example` в `config.json`.
